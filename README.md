[![MasterHead](https://firebasestorage.googleapis.com/v0/b/flexi-coding.appspot.com/o/dempgi7-520f8d5f-63d4-4453-8822-dbc149ae27f8.gif?alt=media&token=91c0c7b2-93c3-4029-b011-1a8703c5730d)](https://github.com/Prabakara-N)
<h1 align="center">Hi <img src="https://user-images.githubusercontent.com/18350557/176309783-0785949b-9127-417c-8b55-ab5a4333674e.gif" alt="title" width="50px" />, I'm Prabakaran</h1>

<h3 align="center">Full-Stack Software Engineer · AI SaaS Builder</h3>

-------------
<img align="right" alt="Coding" width="360" height="240" src="https://cdn.dribbble.com/users/1162077/screenshots/3848914/programmer.gif">

<h4>Hi there 👋</h4>


* 🌍 I'm based in Coimbatore, India
* 🖥️ Check out my portfolio: [**My Portfolio**](https://prabakarandev.in/)
* 🎨 Founder & sole engineer of [**CrayonSparks**](https://www.crayonsparks.com/) — a live AI Book SaaS
* ✉️ Reach me at: [**prabakaran.m0208@gmail.com**](mailto:prabakaran.m0208@gmail.com)
* ⚡ Full-Stack Software Engineer with **3 years** of experience shipping production web apps end-to-end. I love turning ideas into well-architected, AI-powered products and I thrive in dynamic, fast-paced environments.


## 🚀 About Me
I'm a **Full-Stack Software Engineer** passionate about building **type-safe, AI-powered web applications** and **delightful interfaces** that make a difference. I ship products end-to-end — from architecture and backend to deployment — and I'm always exploring better ways to build. Currently open to new full-time opportunities.

---

## 💼 Professional Experience

### Software Engineer @ Darthwares (Jul 2023 – Present)
For 3 years I've built **responsive, scalable web applications** with **Next.js, React, TypeScript, Tailwind CSS, tRPC and oRPC**. I specialize in **type-safe APIs**, **role-based access control (RBAC)**, **real-time features with Firebase**, and **AI-powered modules** serving **2,500+ users** across enterprise and ed-tech products.

I focus on **clean architecture, performance optimization, and seamless user experiences**. Beyond coding, I drive **code reviews**, follow **best practices**, and continuously explore **new tools and frameworks** to ship faster.

#### 🔑 Key Contributions:
1. Built **responsive, performant UIs** with Next.js + Tailwind CSS, improving page load speeds by **40%**
2. Implemented **end-to-end type safety** with tRPC / oRPC, eliminating runtime API errors
3. Engineered **role-based access control** and real-time features with **Firebase** (auth, Firestore, security rules)
4. Developed **AI-powered modules** (resume generation, interactive tutoring, mock placement drives), boosting engagement by **35%**
5. Designed **reusable component architecture** and added **Jest** tests, accelerating delivery and reducing regressions

---

## 🌟 Featured Projects

| Project | Description | Tech |
| --- | --- | --- |
| [**CrayonSparks**](https://www.crayonsparks.com/) | AI Book SaaS (Founder & Sole Engineer) — turns prompts into print-ready story, coloring & activity books | Next.js 16 · React 19 · oRPC · Firebase · OpenAI + Gemini · LemonSqueezy · Cloudflare R2 |
| [**LeetCampus**](https://www.leetcampus.com/) | AI-powered college academic & placement platform serving 2,500+ students | Next.js · TypeScript · Firebase · tRPC · Jotai |
| [**LeetCV**](https://www.leetcv.com/) | AI-powered resume builder with shareable Leet Link | Next.js · TypeScript · Firebase · tRPC · Recoil |
| [**Ennuviz**](https://www.ennuviz.com/) | Enterprise digital-transformation website | Next.js · TypeScript · Sanity CMS · Tailwind CSS |

---

## 🛠️ Languages, Frameworks & Tools

<p align="left">
  <img src="https://skillicons.dev/icons?i=react,nextjs,javascript,typescript,html,css,tailwind,sass,redux,bootstrap" />
</p>
<p align="left">
  <img src="https://skillicons.dev/icons?i=nodejs,express,firebase,mongodb,jest,git,github,vscode,cloudflare,postgresql,mysql" />
</p>
<p align="left">
  <img src="https://skillicons.dev/icons?i=vercel,netlify,postman" />
</p>

**APIs & Architecture:** tRPC · oRPC · REST APIs · Server Actions · Firebase Admin · Webhooks · Sanity CMS

**AI & Integrations:** OpenAI · Google Gemini · Vercel AI SDK · LemonSqueezy & Razorpay (billing)

**Cloud & Storage:** Vercel · Netlify · Cloudflare R2 · AWS S3 · Cloudinary · Sentry

---

### Socials

<p align="left">
  <a href="https://www.github.com/Prabakara-N" target="_blank" rel="noreferrer"> <picture> <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/socials/github-dark.svg" /> <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/socials/github.svg" /> <img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/socials/github.svg" width="38" height="38" /> </picture> </a>
  <a href="https://prabakaran55.hashnode.dev" target="_blank" rel="noreferrer" style="margin-left: 5px;"> <picture> <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/socials/hashnode.svg" /> <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/socials/hashnode.svg" /> <img src="https://raw.githubusercontent.com/danielcranney/readme-generator/main/public/icons/socials/hashnode.svg" width="38" height="38" /> </picture> </a>
  <a href="https://www.instagram.com/the_practical_dev" target="_blank" rel="noreferrer"> <picture> <source media="(prefers-color-scheme: dark)" srcset="https://res.cloudinary.com/daxmjqsy2/image/upload/v1704109502/rh3g50osklnf2lsxng4k.png" /> <source media="(prefers-color-scheme: light)" srcset="https://res.cloudinary.com/daxmjqsy2/image/upload/v1704109502/rh3g50osklnf2lsxng4k.png" /> <img src="https://res.cloudinary.com/daxmjqsy2/image/upload/v1704109502/rh3g50osklnf2lsxng4k.png" width="44" height="44" /> </picture> </a>
  <a href="https://www.linkedin.com/in/prabakaran0208/" target="_blank" rel="noreferrer"> <picture> <source media="(prefers-color-scheme: dark)" srcset="https://res.cloudinary.com/daxmjqsy2/image/upload/v1704109306/snr2yxxlqrmivdtlfdyg.png" /> <source media="(prefers-color-scheme: light)" srcset="https://res.cloudinary.com/daxmjqsy2/image/upload/v1704109306/snr2yxxlqrmivdtlfdyg.png" /> <img src="https://res.cloudinary.com/daxmjqsy2/image/upload/v1704109306/snr2yxxlqrmivdtlfdyg.png" width="44" height="44" /> </picture> </a>
</p>
